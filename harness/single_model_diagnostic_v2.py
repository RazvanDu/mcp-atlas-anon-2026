# single_model_diagnostic_v2.py
#
# Description:
# Single-model diagnosis pipeline with enriched trajectory and v2 failure taxonomy.
#   - Uses enriched agent trajectory (turn structure, tool status, output summaries)
#   - 13 failure modes in 2 categories: tool call issues (5) + cognitive issues (8)
...
#   --scored-file="runs/scored_public_subset/<model>.csv" \
#   --verbose
...
class AsyncLiteLLMClient:
    ...
    def __init__(self, config: DiagnosisConfig):
        ...
        self.base_url = (config.base_url or os.getenv("LITELLM_BASE_URL", "")).rstrip("/")
        raw_keys = config.api_key or os.getenv("LITELLM_API_KEY", "")
        self.api_keys = [k.strip() for k in raw_keys.split(",") if k.strip()]
        if not self.api_keys:
            raise ValueError("LiteLLM API key not found. Set LITELLM_API_KEY env var or pass --api-key.")
        if not self.base_url:
            raise ValueError("LiteLLM base URL not found. Set LITELLM_BASE_URL env var.")
...
class DiagnosticAnalyzer:
    ...
    def _create_diagnosis_prompt(self, task_id: str, prompt: str, agent_trace: str, expected_trajectory: str, gtfa_claims: str, coverage_score: float, missed_claims: List[str], errors_str: str = "") -> str:
        ...
        missed_text = chr(10).join(f'- {claim}' for claim in missed_claims[:20])
...
MISSED CLAIMS ({len(missed_claims)} total):
{missed_text}
...
JUDGE RULES:
1. Pick the most specific mode — use arithmetic_error over faulty_synthesis if the error is a calculation mistake
2. Identify the PRIMARY failure: the single mode that best explains the coverage gap
3. Identify ALL failures present in the trajectory (primary + any contributing failures)
4. For each failure, mark is_root_cause=true if it caused other failures, false if it was downstream
5. faulty_synthesis should only be used when the error is NOT better explained by arithmetic_error, logical_error, or hallucinated_fact
6. Provide a confidence score (0.0-1.0) for your overall diagnosis
7. Write a 1-2 sentence summary explaining what went wrong — this will appear in customer reports

IMPORTANT: This task scored below 1.0, meaning at least one claim was not satisfied. You MUST select a failure mode. If the model got close but produced a subtly wrong answer, use the most specific cognitive mode that applies."""

    async def diagnose_failure(self, task_id: str, prompt: str, expected_trajectory: str, raw_conversation: str, gtfa_claims: str, coverage_score: float, coverage_details: Dict, enriched_trajectory: str = "", errors_str: str = "") -> Dict:
        ...
        if coverage_details and isinstance(coverage_details, dict):
            ...
            for claim_detail in per_claim:
                if claim_detail.get("score", 0) < 0.9:
                    missed_claims.append(claim_detail.get("claim", ""))
...
async def run_diagnosis_pipeline(df: pd.DataFrame, config: DiagnosisConfig, failure_threshold: float = 1.0) -> pd.DataFrame:
    ...
    for idx, row in df[null_mask].iterrows():
        ...
        df.loc[idx, 'diagnosis_primary_mode'] = 'tool_error'
        df.loc[idx, 'diagnosis_primary_category'] = 'tool_call'
        df.loc[idx, 'diagnosis_primary_explanation'] = explanation
        df.loc[idx, 'diagnosis_all_failures'] = json.dumps([{"mode": "tool_error", "category": "tool_call", "is_root_cause": True, "explanation": explanation}])
    ...
    analyzer = DiagnosticAnalyzer(client)

    # Find the enriched trajectory column
    traj_col = next((c for c in df.columns if c == 'model_trajectory' or (c.endswith('_trajectory') and c != 'TRAJECTORY')), None)
    if traj_col:
        logger.info(f"Using enriched trajectory from column: {traj_col}")
    else:
        logger.info("No enriched trajectory column found — will fall back to raw_conversation_history")
    ...
    async def diagnose_row(idx, row):
        ...
        enriched_trajectory = ''
        if traj_col:
            val = row.get(traj_col, '')
            if pd.notna(val):
                enriched_trajectory = str(val)

        errors_str = str(row.get('errors', ''))

        # Short-circuit: no conversation and no trajectory
        if not str(raw_conversation).strip() and not enriched_trajectory.strip():
            return idx, {
                "primary_failure": {"mode": "tool_error", "category": "tool_call", "explanation": _get_error_explanation(row.get('errors'))},
                "all_failures": [],
                "confidence": 1.0,
    ...
    for idx, diagnosis in results:
        ...
        df.loc[idx, 'diagnosis_primary_mode'] = pf.get('mode')
        df.loc[idx, 'diagnosis_primary_category'] = pf.get('category')
...
async def create_diagnosis_summary(df: pd.DataFrame, model_name: str, output_path: str, error_stats: dict, failure_threshold: float, client: Optional['AsyncLiteLLMClient'] = None) -> dict:
    ...
    if 'diagnosis_primary_mode' in df.columns:
        dist = df['diagnosis_primary_mode'].value_counts()
        primary_dist = {str(k): int(v) for k, v in dist.items() if pd.notna(k)}

    # Category split
    tool_call_count = 0
    cognitive_count = 0
    if 'diagnosis_primary_category' in df.columns:
        cat_counts = df['diagnosis_primary_category'].value_counts()
        tool_call_count = int(cat_counts.get('tool_call', 0))
        cognitive_count = int(cat_counts.get('cognitive', 0))
...
async def main(args):
    ...
    try:
        ...
        df_diagnosed, diagnosis_client = await run_diagnosis_pipeline(
            df, config, args.failure_threshold
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Single-model diagnosis with v2 failure taxonomy (13 modes, 2 categories)."
    )

    parser.add_argument("--scored-file", type=str, required=True, help="Path to scored CSV file")
    parser.add_argument("--evaluator-model", type=str, default="gemini/gemini-3.1-pro-preview", help="LiteLLM model name for diagnosis (default: gemini/gemini-3.1-pro-preview)")
    parser.add_argument("--api-key", type=str, default=None, help="LiteLLM API key (or set LITELLM_API_KEY env var)")
    parser.add_argument("--base-url", type=str, default=None, help="LiteLLM proxy base URL (or set LITELLM_BASE_URL env var)")
