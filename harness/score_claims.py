...
#
# Description:
# Re-runs the claim evaluator over an existing scored or unscored CSV of model
# outputs, scoring each task's response against its atomic claims list.
#
# Required files:
#   - Ground-truth CSV with columns: TASK, PROMPT, GTFA, GTFA_CLAIMS
#     (use data/public_split.csv from this release)
#   - Model output CSV with columns: task_id (or TASK), trajectory, response
#
# Example Usage:
#   python score_claims.py \
#     --groundtruth-file="data/public_split.csv" \
#     --model-file="<your scored CSV>" \
#     --num-tasks=100
#
# Override model name via --model-name; otherwise auto-extracted from filename.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Model name for evaluation (LiteLLM format, e.g. gemini/gemini-3.1-pro-preview)."
    )
    parser.add_argument(
        "--api-key",
        type=str,
        default=None,
        help="LiteLLM API key. If not provided, uses LITELLM_API_KEY environment variable."
    )
    parser.add_argument(
        "--base-url",
        type=str,
        default=None,
        help="LiteLLM proxy base URL. If not provided, uses LITELLM_BASE_URL environment variable."
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of concurrent requests to the Gemini API. Auto-set per model if not specified (flash=15, pro=3)."
    )
    parser.add_argument(
        "--num-tasks", 
        type=int, 
